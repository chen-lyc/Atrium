// Shows the chat connection state indicator and success pulse ring.
(() => {
    const { useEffect, useRef, useState } = window.React;
    const { motion, AnimatePresence } = window;
    const { EASE, STATUS_LABEL, STATUS_COLOR } = window.AppConstants;

    function ConnectionStatus({ state, allowPulse = true }) {
