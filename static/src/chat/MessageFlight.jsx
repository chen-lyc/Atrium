...
export default function MessageFlight({ flight, onComplete }) {
  if (!flight) {
  ...
  return (
    <AnimatePresence initial={false}>
      <motion.div
        ...
        animate={
          targetRect
            ? { x: targetRect.left - startRect.left, y: targetRect.top - startRect.top, opacity: 1 }
            : { x: 0, y: 0, opacity: 0.96 }
        }
        exit={{ opacity: 0, transition: { duration: 0.08, ease: EASE } }}
        transition={targetRect ? flightTransition : { duration: 0.06, ease: EASE }}
        onAnimationComplete={() => {
          if (targetRect) onComplete(id);
        }}
      >
