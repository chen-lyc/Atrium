...
(() => {
    ...
    function useAutoTyping({ currentScript, enabled }) {
        ...
        function setCurrentCandidate(nextValue) {
            currentCandidateRef.current = nextValue;
        ...
        function clearTypeBTimer() {
            window.clearTimeout(typeBTimerRef.current);
        ...
        function scheduleTypeB(delayMs = 0) {
            ...
            if (!enabledRef.current || inputValueRef.current.trim() || sendStateRef.current !== "idle") {
                return;
            }
        ...
        useEffect(() => {
            ...
            demoMessageCountRef.current = 0;
            lastScriptMessageIndexRef.current = null;
            scriptMessageIndexAtSuggestionRef.current = null;
        ...
        useEffect(() => {
            ...
            function handleDemoBroadcast(event) {
                ...
                if (detail.source !== "demo" || detail.isSelf || !detail.isScriptMessage) {
                    return;
                }

                demoMessageCountRef.current += 1;
                lastScriptMessageIndexRef.current =
                    typeof detail.messageIndex === "number" ? detail.messageIndex : null;

                if (!enabledRef.current || sendStateRef.current === "user-editing") {
                    return;
                }

                if (sendStateRef.current === "typing" || sendStateRef.current === "deleting") {
                    return;
                }
                ...
                if (sendStateRef.current === "waiting" && inputValueRef.current.trim()) {
                    ...
                    const messagesSinceSuggestion = Math.max(
                        0,
                        demoMessageCountRef.current - messageCountAtSuggestionRef.current
                    );
                    const scriptAdvance =
                        typeof lastScriptMessageIndexRef.current === "number" &&
                        typeof scriptMessageIndexAtSuggestionRef.current === "number"
                            ? lastScriptMessageIndexRef.current - scriptMessageIndexAtSuggestionRef.current
                            : messagesSinceSuggestion;
                    const minVisible =
                        candidate.kind === "typeA" ? TYPE_A_MIN_VISIBLE_MS : TYPE_B_MIN_VISIBLE_MS;
                    const minScriptAdvance =
                        candidate.kind === "typeA"
                            ? TYPE_A_MIN_SCRIPT_ADVANCE
                            : TYPE_B_MIN_SCRIPT_ADVANCE;

                    if (suggestionAge < minVisible || scriptAdvance < minScriptAdvance) {
                        return;
                    }

                    const shouldOfferNext = candidate.kind !== "typeA";
                    startDeleting(() => {
                        if (shouldOfferNext && demoMessageCountRef.current % 2 === 0) {
                ...
                if (
                    ...
                    hasTriggeredTypeARef.current &&
                    demoMessageCountRef.current % 3 === 0
                ) {
                    scheduleTypeB(reducedMotion ? 0 : 900);
                }
            }
            ...
            return () => {
                window.removeEventListener("signal-message-broadcast", handleDemoBroadcast);
            };
