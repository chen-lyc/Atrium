...
(() => {
    ...
    function dispatchMessageSignal(message, meta = {}) {
        if (message.nickname === "__system__" || message.nickname === "system") {
            window.dispatchEvent(
                new CustomEvent("signal-message-system", {
                    detail: {
        ...
        window.dispatchEvent(
            new CustomEvent("signal-message-broadcast", {
                detail: {
                    messageId: message.id,
                    nickname: message.nickname,
                    source: message.source,
    ...
    function dispatchScriptStart(scriptIndex) {
        window.setTimeout(() => {
            window.dispatchEvent(
                new CustomEvent("signal-script-start", {
                    detail: { scriptIndex }
                })
            );
        }, 0);
    ...
    function useDemoMessages({
        enabled,
        fadeDuration = 600,
        onTypeATrigger,
        reducedMotion = false,
        preserveMessagesOnDisable = false
    }) {
        ...
        function enqueueMessage(message, afterCommit, meta) {
            messageQueueRef.current.push({ message, afterCommit, meta });
            processQueue();
        }
        ...
        function schedulePendingMessage(scriptIndex, messageIndex, delayOverride) {
            ...
            const delay =
                delayOverride != null
                    ? delayOverride
                    : messageIndex === 0
                        ? Math.max(INITIAL_SCRIPT_LEAD_IN_DELAY, baseDelay)
                        : baseDelay;
        ...
        function startFade(scriptIndex, delayOverride) {
            ...
            scheduleCore(delay, () => {
                ...
                dispatchScriptStart(nextIndex);
                startSwitchClear(nextIndex);
            });
        }

        function startSwitchClear(scriptIndex, delayOverride) {
            const delay =
                delayOverride != null ? delayOverride : reducedMotionRef.current ? 0 : SCRIPT_CLEAR_SETTLE_DELAY;

            controllerRef.current.scriptIndex = scriptIndex;
            controllerRef.current.messageIndex = 0;
            controllerRef.current.phase = "clear";

            scheduleCore(delay, () => {
                setFading(false);
                schedulePendingMessage(scriptIndex, 0);
            });
        }
        ...
        function resumeCore() {
            ...
            if (controller.phase === "clear") {
                startSwitchClear(controller.scriptIndex, controller.remaining);
            }
        }
        ...
        function appendInternalMessage(message, afterCommit, meta) {
            enqueueMessage(message, afterCommit, meta);
        }
        ...
        function holdScript(pauseScriptMs = 0) {
            if (pauseScriptMs > 0) {
                pauseCore(pauseScriptMs);
            }
        }
